...
function normalizeSizeLimits({minWidth = 0, minHeight = 0, maxWidth = Infinity, maxHeight = Infinity} = {}) {
  ...
  return {minWidth: _minWidth, minHeight: _minHeight, maxWidth: _maxWidth, maxHeight: _maxHeight}
}

// 是否存在最小尺寸限制（用于决定是否禁用翻转）
function hasMinSizeLimit(limits) {
  return limits.minWidth > 0 || limits.minHeight > 0
}

// 锁定比例时，保证比例为有限正数；否则回落 1
function getValidWhRatio(whRatio) {
  return isFinite(whRatio) && whRatio > 0 ? whRatio : 1
}

// 根据限制生成最终宽高；lock 时会同步折算另一边的限制，保持宽高比
function getLimitedSize({width, height, lock = false, baseAxis = 'width', limits, whRatio = 1}) {
...
export { clampSize, getLimitedSize, getValidWhRatio, hasMinSizeLimit, normalizeSizeLimits }
