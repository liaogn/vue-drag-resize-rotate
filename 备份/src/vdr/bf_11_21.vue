...
export default {
  ...
  mounted() {
    ...
    if (!window.vdrArrowIcon) {
      window.vdrArrowIcon = document.createElement('i')
      window.vdrArrowIcon.setAttribute('id', 'vdrArrowIcon')
      document.body.appendChild(window.vdrArrowIcon)
    }
  },
  beforeDestroy() {
    // 销毁前移除事件
  ...
  methods: {
    ...
    stickDownInit(stick) {
      ...
      // 记录临界点(用于处理缩放时的临界翻转或限制)
      this.breakingPoints = this.getBreakPoints(stick)
      this.breakingFlags = this.breakingPoints.map((point) => {
        const curPoint = this.absoluteContactor
        const diff = point[0] - curPoint[0]
        return diff !== 0 ? diff > 0 : point[1] >= curPoint[1]
      })

      // 记录临界点直线方程
      this.breakingLines = this.breakingPoints.map((point) =>
        this.lineEquation(point, this.symAbsoluteContactor)
      )
      // console.log(this.breakingPoints)
      // console.log(this.breakingLines, this.breakingFlags)

      // console.log(
      //   lineEquation(this.breakingPoints[0], this.symAbsoluteContactor)
      // )
    ...
    getBreakPoints(stick) {
      ...
      return breakSticks.map((breakStick) =>
        this.calcRotatedContactor(this.elementInfo, breakStick)
      )
    },
    ...
    getFlipStick(stick) {
      ...
      if(stick.match('m')){
        // return stick
        const reverseMap = {tm: 'bm', bm: 'tm', ml: 'mr', mr: 'ml'}
        return `${reverseMap[stick]}`
      }else{
        const reverseMap = {t: 'b', b: 't', l: 'r', r: 'l'}
    ...
    // 进行翻转
    flipRect(stick) {
      // if (this.flipRectTimer) return
      // console.log(stick,'stick',index);
      const flipStick = this.getFlipStick(stick)
      // index!==1 && (this.rotate += 180)
      // this.rotate += 180
      console.log('flipStick',flipStick);
      // console.log(flipStick,'flipStick');
      // this.$nextTick(() => {
      ...
      // })
      // this.flipRecting = false
      // clearTimeout(this.flipRectTimer)
      // this.flipRectTimer = null
    },
    ...
    stickMove(ev) {
      ...
      const {breakingPoints, breakingFlags} = this

      if (this.lock || this.isMiddlePoint) {
        // 当拖拽触点为中点（tm,bm,mr,ml）时或锁定比例时
      ...
      // 遍历临界直线，判断鼠标点是否超出临界直线
      // this.breakingLines.forEach((func, index) => {
      //   if (this.flipRecting) return
      //   // console.log('判断');
      //   // 先判断x，再判断y（x或y点代入直线方程进行对比）
      //   const diff_x = func('x', mousePoint[1]) - mousePoint[0]
      //   if (diff_x !== 0 && !isNaN(diff_x)) {
      //     breakingFlag = diff_x > 0
      //   } else {
      //     const value_y = func('y', mousePoint[0]) || breakingPoints[index][1]
      //     breakingFlag = value_y > mousePoint[1]
      //   }
      //   if (breakingFlags[index] !== breakingFlag) {
      //     console.log('触发翻转', ++this.num)
      //     this.flipRecting = true
      //     this.flipRect(this.currentStick)
      //   }
      // })
    ...
    //     // 求点p0在p1与p2构成的直线的上哪一侧
    //     calePoinWhereSideLine(p0, p1, p2){
    //
    //     },
    // 获取中心点
    ...
    // 获取触点中点坐标（相对文档视口）
    getTestStickPoint(stick) {
      const symStick = this.$refs[`stick_${stick}`][0]
      const symStickInfo = symStick.getBoundingClientRect()
      return [
        symStickInfo.left + symStickInfo.width / 2,
        symStickInfo.top + symStickInfo.height / 2,
      ]
    },
  },
  watch: {},
