...
export default {
  ...
  methods: {
    ...
    stickDown(ev, stick, index) {
      ...
      // 计算当前拖拽点的对称点（相对文档左上角，已验证，ok）
      this.symAbsoluteContactor = this.calcRotatedContactor(
        this.elementInfo,
        this.symCurrentStick
      )

      // 计算对称点基于父元素中点回复未旋转时、以及当前元素未旋转时的点（相对文档左上角，已验证，ok）
      ...
      // console.log(this.symRotatedContactor)
      ...
      const symStick = this.$refs[`stick_${this.symCurrentStick}`][0]
      ...
      // console.log(this.symAbsoluteContactor, '现：相对视口')
      // console.log(symPointCenter, '原：相对视口')
      this.symPoint = [
        this.getRotatedRelativeX(this.parentInfo, symPointCenter),
        this.getRotatedRelativeY(this.parentInfo, symPointCenter),
      ]
      console.log(this.symPoint, '原')
      console.log(this.symRelativeContactor, '现')
    },
    ...
    stickMove(ev) {
      ...
      const {cx, cy, absoluteRotate, left, top} = this.parentInfo
      const mouseAbsoluteRotatedPoint = this.calcRotatedPoint(
        [ev.clientX, ev.clientY],
        [cx, cy],
        absoluteRotate - this.elementInfo.rotate
      )
      const mouseRelativeRotatedPoint = [
        mouseAbsoluteRotatedPoint[0] - left,
        mouseAbsoluteRotatedPoint[1] - top,
      ]
      // console.log(mousePoint, 'mousePoint')
      // console.log(mouseRelativePoint, 'mouseRelativePoint')
      // let mousePoint = {x: ev.clientX, y: ev.clientY}
      // let mouseRelativePoint = [
      //   this.getRotatedRelativeX(this.parentInfo, mousePoint),
      //   this.getRotatedRelativeY(this.parentInfo, mousePoint),
      // ]
      ...
      // console.log(mouseRelativePoint, newCenterPoint)
      this.topLeftStick(mouseRelativeRotatedPoint, newCenterPoint)
    },
    ...
    topLeftStick(curPositon, newCenterPoint) {
      ...
      let newWidth = newBottomRightPoint[0] - newTopLeftPoint[0]
      let newHeight = newBottomRightPoint[1] - newTopLeftPoint[1]
      console.log(newTopLeftPoint, newBottomRightPoint)
      this.width = newWidth
      this.height = newHeight
      this.left = newTopLeftPoint[0]
      this.top = newTopLeftPoint[1]
    },
