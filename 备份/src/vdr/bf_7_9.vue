...
export default {
  ...
  methods: {
    ...
    stickDown(ev, stick, index) {
      ...
      // 获取画布位移信息
      this.parentInfo = this.parentElement.getBoundingClientRect()
      this.parentsRotate = this.getParentsRotate(ev, true)
      // console.log(this.parentsRotate, '父元素叠加角')
      this.stickInfo = ev.target.getBoundingClientRect()

      // this.stickInfoCenter = {
      //   x: this.stickInfo.left + this.stickInfo.width / 2,
      //   y: this.stickInfo.top + this.stickInfo.height / 2,
      // }
      ...
      // 当前点击坐标
      // this.curPoint = {
      //   x: this.stickInfoCenter.x - this.parentInfo.left,
      //   y: this.stickInfoCenter.y - this.parentInfo.top,
      // }
      // // // 获取对称点的坐标
      // this.symmetricPoint = {
      ...
      // }
      this.symmetricPoint = {
        x: symStickInfo.left + symStickInfo.width / 2 - this.parentInfo.left,
        y: symStickInfo.top + symStickInfo.height / 2 - this.parentInfo.top,
      }

      console.log(this.symmetricPoint, '点击时的对角点')
      // 触发事件
    ...
    calculateRotatedPointCoordinate(point, center, rotate) {
      ...
      return {
        x:
          (point.x - center.x) * Math.cos(this.angleToRadian(rotate)) -
          (point.y - center.y) * Math.sin(this.angleToRadian(rotate)) +
          center.x,
        y:
          (point.x - center.x) * Math.sin(this.angleToRadian(rotate)) +
          (point.y - center.y) * Math.cos(this.angleToRadian(rotate)) +
          center.y,
      }
    },
