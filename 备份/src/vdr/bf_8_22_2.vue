...
export default {
  ...
  methods: {
    ...
    bodyMove(ev) {
      ...
      // x'=x·cos(θ)+y·sin(θ)
      // y'=y·cos(θ)-x·sin(θ)
      const rad = this.getRad(this.parentsRotate)
    ...
    stickDown(ev, stick, index) {
      ...
      this.breakStick1Contactor = this.calcRotatedContactor(
        this.elementInfo,
        this.breakStick1
      )
      // console.log(this.getTestStickPoint('bl'))
      this.breakStick2Contactor = this.calcRotatedContactor(
        this.elementInfo,
        this.breakStick2
      )
    ...
    stickMove(ev) {
      ...
      let b1 = this.calcVerticalCrossPoint(
        mousePoint,
        this.breakStick1Contactor,
        this.symAbsoluteContactor
      )
      // let k = (mousePoint[1] - b1[1]) / (mousePoint[0] - b1[0])
      const s1 = String(mousePoint[0] - b1[0]).replace(/(\d+)/, '') || '+'
      const s2 = String(mousePoint[1] - b1[1]).replace(/(\d+)/, '') || '+'
      console.log(ev.target)
      if (this.curSign && this.curSign[0] !== s1 && !this.flag) {
        console.log('临界')
        this.flag = true
        this.rotate += 180
        this.$nextTick(() => {
      ...
      console.log(this.curSign)

      if (this.isMiddlePoint || this.lock) {
      ...
      this.caclRectResetRotated(
        mouseRelativeRotatedPoint,
        newCenterPoint,
        this.flag ? 'tr' : this.currentStick
      )
    },
