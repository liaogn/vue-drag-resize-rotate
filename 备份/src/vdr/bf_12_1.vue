...
const updateRactMaps = {
  ...
  tr(point, symPoint,lock,c) {
    ...
    const diff = (width - 150)/2
    console.log(c);
    // console.log(c,'newCenterPoint');
    // if(width>=150)return console.log('stop');
    if(width>=150){
  
      const x = diff*Math.cos(this.rad)
      const y = diff*Math.sin(this.rad)
      console.log(y,'y');
      const nc = [cx-x,cy-y]
      this.width = 150
      this.height = height
      this.left = nc[0] - 150/2
